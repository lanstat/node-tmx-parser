...
describe("tile encoding types", function() {
  ...
  it("b64-zstd", generateEncodingTypeTest("lemming-level3-b64zstd.tmx"));
});
...
describe("tilesets", function() {
  ...
  it("external tileset", function(done) {
    var target = path.join(__dirname, "external-tileset.tmx");
    tmx.parseFile(target, function(err, map) {
      if (err) return done(err);
      assert.strictEqual(map.tileSets.length, 1);
      assert.strictEqual(map.tileSets[0].firstGid, 1);
      assert.strictEqual(map.tileSets[0].source, "../test/tiles.tsx");
      done();
    });
  });
});
